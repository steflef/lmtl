...
eventManager.prototype.parseEvents = function (e, l, onComplete) {
    ...
    (function () {
        ...
        for (var i = pos; i < s; i++) {
            ...
            t.point = new OpenLayers.LonLat(t.lon, t.lat).transform(serverProj, map.getProjectionObject());
            //t.point = new OpenLayers.LonLat( t.lon, t.lat );
            t.px = map.getLayerPxFromLonLat(t.point);
            l_up[t.id] = i;
        }
...
eventManager.prototype.runReset = function (e, l, onComplete){
    ...
    (function(){
		...
		for(var i= pos;i<s;i++)
		{
			...
			o.visible = (o.err)?0:1;
			o.cluster = null;
			o.px = map.getLayerPxFromLonLat(o.point);
		}
        pos = s;
        if (pos < l)
        {
...
eventManager.prototype.drawGeneratedClusters =function(new_zoom)
{
	...
	(function(){
		...
		var c_div = $j(map.layerContainerDiv);
		var ex = map.getExtent().transform( map.getProjectionObject(),serverProj);
		...
		////console.log(c_lon + ", "+ c_lat);
			...
			// IN BOUNDS ONLY
			if(c_lat < ex.top && c_lat > ex.bottom && c_lon > ex.left && c_lon < ex.right && vClusters[i]== undefined)
			{
				// RAPHAEL CIRCLE
				var lonlat = new OpenLayers.LonLat( c_lon, c_lat); 
				var px = map.getLayerPxFromLonLat( lonlat.transform(serverProj, map.getProjectionObject()) );

				// Creates circle
				var rad = Math.abs(z[i].len * 0.4) +25 ;
