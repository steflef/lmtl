 ...
 * @type {string}
 */
var FUSION_URL = 'https://www.google.com/fusiontables/api/query';
...
function init() {
    ...
    var lastHeaderValue = sheet.getRange(1, lastColumn)
        .getValue();
    if (lastHeaderValue != 'rowid') {
        sheet.getRange(1, lastColumn + 1)
            .setValue('rowid');
    }
}
...
function run(query) {
    ...
    if (lowercaseQuery.indexOf('select') == 0 || lowercaseQuery.indexOf('show') == 0 || lowercaseQuery.indexOf('describe') == 0) {
        method = 'get';
    }
    ...
    try {
        ...
        if (err.message.search('401') != -1) {
            ...
            if (err.message.search('Bad column reference') != -1) {
                Logger.log('Looks like the column names in the form do not match ' + 'the column names in the table. Make sure these match!');
            }
            var addressColumn = ScriptProperties.getProperty('addressColumn');
            var latlngColumn = ScriptProperties.getProperty('latlngColumn');
            if (addressColumn && !latlngColumn) {
                Logger.log('Since you added an addressColumn project property, ' + 'you also need to add a latlngColumn property');
            }
...
function getGAauthenticationToken() {
    ...
    try {
        response = UrlFetchApp.fetch('https://www.google.com/accounts/ClientLogin', {
            method: 'post',
            payload: 'accountType=GOOGLE&Email=' + username + '&Passwd=' + password + '&service=fusiontables&Source=googledocs'
        });
    } catch (err) {
        Logger.log('Error authenticating.');
        Logger.log(err.message);
